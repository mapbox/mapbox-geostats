  ...
  function getSource() {
    return new Promise((resolve, reject) => {
      new MBTiles(filePath, (err, source) => {
        console.log('did we set up an mbtiles instance?');
    ...
    return new Promise((resolve, reject) => {
      ...
      zxyStream.on('error', reject)
        ...
        .on('data', (d) => {
          count++;
          console.log(count);
          if (count === Constants.MBTILES_MAX_TILE_COUNT) {
            readStream.push(null); // this kills the stream
          }
        })
        .on('error', (err) => {
          if (/EOF/.test(err.message)) {
